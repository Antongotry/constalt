...
(function () {
  ...
  function initHeaderScrollState() {
    ...
    var logo = header.querySelector('.site-header__logo[data-header-logo-dark]');
    var darkSrc = logo ? logo.getAttribute('data-header-logo-dark') : '';
    var lightSrc = logo ? logo.getAttribute('data-header-logo-light') : '';
    var threshold = 32;
    ...
    function tick() {
      ...
      if (logo && darkSrc && lightSrc) {
        var next = scrolled ? lightSrc : darkSrc;
    ...
    var lenisRef = window.__constaltLenis;
  ...
  function initBlogArchiveFilters() {
    ...
    function getSlugFromHref(href) {
      ...
      var matched = value.match(/\/category\/([^\/?#]+)\/?/i);
    ...
    function getFilterSlug(filterNode) {
      ...
      if (filterNode.tagName === 'A') {
        return getSlugFromHref(filterNode.getAttribute('href'));
      }
    ...
    function getPostCategories(postNode) {
      ...
      if (rawValue) {
        categories = rawValue.split(/\s+/).map(function (slug) {
      ...
      postNode.querySelectorAll('.blog-post__category, a[href*="/category/"]').forEach(function (categoryLink) {
        ...
        if (categoryLink.tagName === 'A') {
          slug = getSlugFromHref(categoryLink.getAttribute('href'));
        }

        if (!slug) {
          slug = String(categoryLink.textContent || '').trim().toLowerCase();
        }
  ...
  runInit(initHeaderScrollState, 'header-scroll');
